...
class Transactions extends CI_Controller{
    ...
    */
    ...
    public function index(){
        $data['items'] = $this->item->getActiveItems('name', 'ASC');//get items with at least one qty left, to be used when doing a new transaction
        
        // $data['pageContent'] = $this->load->view('transactions/transactions', $transData, TRUE);
        // $data['pageTitle'] = "Transactions";
        // $this->load->view('main', $data);
        render('transactions/transactions', $data);
    }
    ...
    */
    ...
    public function nso_(){
        ...
        if($allIsWell){//insert each sales order into db, generate receipt and return info to client
            ...
            $eventDesc = count($arrOfItemsDetails). " items totalling KES". number_format($cumAmount, 2)
                    ." with reference number {$returnedData['transRef']} was purchased";
            
            $this->genmod->addevent("New Transaction", $returnedData['transRef'], $eventDesc, 'transactions', $this->session->user_id);
       }
