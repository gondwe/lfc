...
class tablo extends fieldsets{
	...
	public function table($display_links = 1){
		...
		echo "<thead>";
			echo "<tr style='background:#800f7b; padding-left:10px; font-weight:lighter;border-right: 1px solid black;' class='text-light'>";
				echo "<th  border-radius:50px;>SNO</th>";
		...
		echo "<tbody>";
			...
			echo $this->edit ? "<td><a data-toggle='tooltip' title='Edit' href='#edit/".$this->table."/".$dd['id']."'><i class='fa fa-edit text-primary' style='color:#800f7b'></i></a></td>" : null;
			$urlx = "#delete/".$this->table."/".$dd['id']."&url=".$_SERVER["QUERY_STRING"];
...
';
	...
	protected function get($i){ 
		$l =[];
		$j = $this->db->query($i) or spill($this->db->error()); 
